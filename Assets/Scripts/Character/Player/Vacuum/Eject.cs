...
public class Eject : MonoBehaviour
{
    ...
    [SerializeField] private EjectOre _ejectOrePrefab;
    ...
    private void Start()
    {
        ...
        _vacuum.VacuumActions.SpittingOut.canceled += _ => CancelBlowOut();
    }
    ...
    private void Update()
    {
        ...
        if (_vacuum.VacuumActions.SpittingOut.IsPressed() && !_absorption.IsSuckUp)
        {
    ...
    private void GenerateTile()
    {
        ...
        var chordLength = Mathf.Sqrt(Mathf.Pow(_vacuumData.EjectDistance, 2) + Mathf.Pow(_vacuumData.EjectRange, 2));
        var angle2 = Mathf.Acos(_vacuumData.EjectDistance / chordLength);
        ...
        if (_blockType is BlockType.Ruby or BlockType.Crystal or BlockType.Emerald)
        {
            ...
            var position = _vacuumData.EjectDistance * _vacuum.Direction.normalized + _vacuum.Pivot.position;
            ...
            blowOutOre.SetOre(ore, _vacuum.Direction.normalized);
    ...
    private void UpdateTiles()
    {
        ...
        var bounds = new BoundsInt(_updateTilemap.WorldToCell(_vacuum.Pivot.position) - new Vector3Int((int)_vacuumData.EjectRadius, (int)_vacuumData.EjectRadius, 0), new Vector3Int((int)_vacuumData.EjectRadius * 2, (int)_vacuumData.EjectRadius * 2, 1));
        ...
        foreach (var position in positions)
        {
            ...
            var direction1 = position - _vacuum.Pivot.position;
            var angle = Vector3.Angle(direction1, _vacuum.Direction);

            var dis = Vector3.Distance(_vacuum.Pivot.position, position);
            if (angle <= 30 && dis <= _vacuumData.EjectRadius)
            {
    ...
    private void SetUpdateTile(Vector3Int position)
    {
        ...
        var direction = position - _vacuum.Pivot.position;
        var newTilePosition = Vector3Int.RoundToInt(position + direction.normalized * _vacuumData.EjectSpeed) + Vector3Int.up * _vacuumData.EjectUp;
    ...
    private void OnDrawGizmos()
    {
        ...
        Gizmos.DrawWireSphere(_vacuum.Pivot.position, _vacuumData.EjectRadius);

        Gizmos.color = Color.blue;
        Gizmos.DrawWireSphere(_vacuum.Pivot.position, _vacuumData.EjectDistance);

        var targetPosition = _vacuum.Pivot.position + _vacuum.Direction.normalized * _vacuumData.EjectDistance;
        Gizmos.DrawLine(_vacuum.Pivot.position, targetPosition);
        ...
        Gizmos.DrawWireSphere(_vacuum.Pivot.position, chordLength);

        var chordLength2 = Mathf.Sqrt(Mathf.Pow(0, 2) + Mathf.Pow(_vacuumData.EjectRange, 2));
        var angle3 = Mathf.Acos(0 / chordLength2);
        var h = _vacuumData.EjectRadius * (1 - Mathf.Cos(angle3));
        var y = Mathf.Sqrt(Mathf.Pow(_vacuumData.EjectRadius, 2) - Mathf.Pow(_vacuumData.EjectRange, 2)) - (_vacuumData.EjectRadius - h) - _vacuumData.EjectDistance;
